...
public class MetamorphicMonsterStone extends UBStone {
  ...
  public float getBlockHardness(IBlockState state, World worldIn, BlockPos pos) {
    return Math.round(getBaseHardness() * state.getValue(METAMORPHIC_VARIANT_PROPERTY).getHardness() * 100f) / 100f;
  }

  @Override
  public float getExplosionResistance(World world, BlockPos pos, @Nullable Entity exploder, Explosion explosion) {
    return Math.round(getBaseResistance() * world.getBlockState(pos).getValue(METAMORPHIC_VARIANT_PROPERTY).getResistance() * 100f) / 100f;
  }
